...
- ⚠️ CORRECTION of this entry's first draft: those numbers compared fsci (rch remote worker)
  vs scipy (local box) — a CROSS-BOX artifact, invalid at a ~1.5× margin (the parallel path
  even mis-measured 155 ms on a contended `vmi` worker vs 78 ms serial on `hz2`). Re-measured
  ALL of fsci serial, fsci parallel, and scipy on the SAME local 64-core box below.
- MEASURED (SAME local box, criterion median, scipy 1.17.1, unsorted+dup triplets, nnz=2M):

  | case (nnz/row) | fsci serial | fsci parallel (shipped) | scipy tocsr+sort_indices | verdict |
  | --- | ---: | ---: | ---: | --- |
  | n=100k (20/row) | 84.89 ms | **68.00 ms** | 46.10 ms | **1.48× slower** |
  | n=20k (100/row) | 85.21 ms | **30.54 ms** | 56.82 ms | **1.86× FASTER** |

  - The parallel path HELPS at both regimes vs serial (1.25× at n=100k, **2.79× at n=20k**) — no
    regression, so the committed gate is correct.
  - fsci **WINS SciPy 1.86×** for moderate-density rows (n=20k, 100 nnz/row) and **loses 1.48×**
    for very-sparse many-row input (n=100k, 20 nnz/row), where the serial O(nnz) histogram+scatter
    (100k random row-writes = cache misses) + concat overhead dominates the tiny per-row sorts.
- Correctness: new `coo_to_csr_counting_sort_matches_dense_*` asserts both serial and parallel
...
## Uncontended-surface saturation sweep + hyperu quadrature lever (filed, not a loss) — AmberKestrel 2026-06-29
Dig after the voigt win. Measured fsci-vs-scipy head-to-head across the uncontended crates to find a LOSS to
flip; found the surface is SATURATED — every function already wins or is covered:
- ndimage.distance_transform_edt 600²/1000²: fsci 8.43/28.74 ms vs scipy 29.3/85.1 ms = **3.0-3.5× WIN**
  (serial Felzenszwalb in Rust beats scipy's C; already had perf_edt.rs — covered).
- ndimage.map_coordinates: already parallel (thread::scope across coords) — covered.
- interpolate.griddata linear/cubic: already parallel (par_query_try_map) — covered.
- special.erf complex (erf_complex_series): already a recurrence + early-break (~15 terms, not 80) — optimized.
- special.hyperu a>0 (n=100k/1M): fsci 52.6/504.2 ms vs scipy 90.1/738.6 ms = **1.47-1.71× WIN** (already
  parallel via par_map_indices, despite a 768-step Simpson per call).

NOT a loss anywhere. The one improvable item: **hyperu's 768-step fixed Simpson** (`hyperu_positive_a_integral`,
the PRIMARY path for a>0) caps the existing 1.5× win. FILED LEVER (the wofz-CF pattern's cousin): the
integrand exp(φ(s)), φ(s)=−x·eˢ+a·s+(b−a−1)·ln(1+eˢ)−lnΓ(a), is single-peaked at peak_s. REJECTED the easy
"Gauss-Legendre over [lower,upper]" idea by analysis: the interval is peak_s±20 (~40 wide) but the peak width
is ~O(1), so the peak is ~1/40 of the interval and GL clusters nodes at the ENDPOINTS, not the central peak —
GL-on-wide-interval would be worse, not better. The CORRECT tool is **Laplace-scaled Gauss-Hermite about
peak_s** (substitute s = s* + u·√(2/−φ''(s*)), ~30-40 GH nodes → ~1e-12, ~20× fewer integrand evals than 768)
→ projects hyperu ~10× vs scipy. Needs Newton-refine s*, φ''(s*), and a GH node table — a real, conformance-
sensitive implementation deferred to a dedicated turn (would lift an already-winning fn, not close a loss).
...
## 2026-06-29 — AmberKestrel (cc): post-GMM cluster sweep — remaining functions blocked or competitive (no clean lever)

After the two GMM M-step wins (c3e8887a full, acacc0be diag), swept the rest of fsci-cluster with the loop-interchange/per-group-parallel audit lever. No further clean win:

- **kernel_pca / spectral_embedding** take a PRECOMPUTED kernel/affinity matrix as input (not raw data) — they only double-center + eigendecompose, so there is no O(n²·d) build to parallelize here. Not a target.
- **stats `cov` functions** are scalar pairwise cov(x,y) / multinomial k×k cov — not a data covariance-matrix builder. No bad loop nest.
- **kmeans** — re-measured at scale (n=100k/d=50/k=20): fsci 514ms/48-iter vs sklearn 319ms/2-iter, but this is a CONVERGENCE-CRITERION artifact, NOT a per-iter speed gap (fsci ~10.7ms/iter is competitive). fsci's check is `|Δinertia| < 1e-12·inertia` — so strict that boundary-point label oscillation never settles and it grinds to ~max_iter, while sklearn's center-shift tol (1e-4) stops early. TWO blockers to "fix" it: (a) fsci kmeans has NO tol parameter (hardcoded 1e-12) — loosening it to scipy's ~1e-5 is an output-changing semantic change, and conformance tests pin exact centroids to 1e-8 (lib.rs:9998) and exact per-iteration labels (9994/10003), so it would break GREEN; (b) the serial M-step centroid accumulation can't be parallelized byte-identically (the comment at 2710-2712 documents that inertia is summed sequentially specifically to keep convergence/iteration-count bit-identical — privatized partials reassociate the sum → different convergence path). REJECTED as too risky for the gain.

CONCLUSION: the fsci-cluster lane is now thoroughly optimized (NMF + GMM-full + GMM-diag shipped this session; kmeans/kernel_pca/spectral/FA/PPCA all competitive-or-blocked). Future kmeans lever would need an API addition (tol param) + matching scipy's center-shift convergence — a conformance-sensitive task for a dedicated turn, not a quick win. Next productive direction is a different crate.

## 2026-06-29 — AmberKestrel (cc): stats + interpolate sweep — all measured fns win or already parallel; the one loss is sub-threshold for profitable parallelism

Pivoted off the (now-dominated) cluster lane to fsci-stats and fsci-interpolate, measuring same-box at scale. Everything wins or is already parallel:
- **theilslopes** n=3000 12.2ms / n=6000 192.8ms vs scipy 201 / 910 = **16.5× / 4.7× FASTER**. (The n=6000 jump is the fast path falling back to the serial O(n²) `theilslopes_materialized` on TIED x-values — synthetic-data artifact; still wins. Parallelizing that fallback like siegelslopes would help the degenerate case only — niche, deferred, NOT a scipy loss.)
- **siegelslopes** 2.9 / 7.2ms vs scipy 153 / 664 = **53× / 92× FASTER** (already parallel across anchors).
- **gaussian_kde** covariance build is good loop order (point-outer, O(n·d²)) and tiny for typical small-d KDE; eval parallel — wins (memory 30×).
- **binned_statistic_dd** already parallel (privatized partials).
- **RegularGridInterpolator.eval_many / interpn** already parallel (`par_query_try_map`, work-gated).
- **CloughTocher2DInterpolator**: eval_many wins **5.7×–51×** (npt=3000/8000, nq=40k/80k: fsci eval 7.8/3.4ms vs scipy 44.8/174ms); **construct is 1.32–1.35× slower** (11.9/34.1 vs 8.8/25.8ms) — the serial per-vertex gradient `.map().collect()` (lib.rs:4620) + per-triangle patch map (4465). REJECTED parallelizing it: n=3000–8000 cheap-per-element maps fall FAR below the crate's measured `PAR_QUERY_MIN_WORK = 1<<23` gate (break-even ~350k elements; the parallel path costs ~4-5ms fixed under fleet contention) → would be ~0-gain or a loss (same class as the reverted 1-D evaluate_many). CT OVERALL wins 5.3× (construct+eval); the construct sub-loss is not profitably fixable on this box.

CONCLUSION: the accessible stats/interpolate hot paths are dominated. The remaining genuine scipy losses in the repo are the documented constant-factor inlined-C walls (filter1d 1D van Herk, jnjnp_zeros, eigsh Lanczos) and probe-crate residuals owned by other agents (linalg/signal/sparse). Session shipped 6 measured wins (lsq_linear, dbscan, NMF×2, GMM-full, GMM-diag); the profitable-loss surface in my non-probe lanes is now thin.

## 2026-06-29 — AmberKestrel (cc): CloughTocher construct parallelization MEASURED as a regression (reverted) + 5-crate domination sweep

Followed up last cycle's threshold-argument with a real test: parallelized the CT construct's per-vertex gradient `.map()` and per-triangle patch `.map()` via a new ordered `par_index_map` with a construct-tuned gate (1<<19, lower than the eval gate 1<<23). MEASURED same-box: npt=8000 construct 34.1 → **40.5 ms (WORSE)** — the ~4-5ms parallel overhead exceeds the gain at the size where the gate activates; npt=3000 stayed serial (gated). Confirms the eval gate's calibration applies here too. REVERTED (git show HEAD:lib.rs). The CT construct 1.35× loss is genuinely not profitably parallelizable on this contended box.

Plus a broad domination sweep this cycle — every all-pairs / iterative / pairwise function checked is ALREADY parallel or already wins:
- cluster: affinity_propagation (parallel, ordered slots), NMF/GMM×2 (shipped this session), kmeans (blocked, prior), kernel_pca/spectral (precomputed input).
- sparse csgraph (linalg.rs): dijkstra_all_pairs / dijkstra_multi_source / johnson / bellman_ford_multi_source all PARALLEL; floyd_warshall PARALLEL (block-pivot `floyd_warshall_blocked`). Single-source dijkstra/bellman_ford correctly serial.
- spatial: directed_hausdorff (Taha-Hanbury early-break + parallel), procrustes (small-d SVD).
