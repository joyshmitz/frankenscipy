...
| `fsci-ndimage` label-indexed measurements performance | **complexity gap closed (label-stats)** | `measurement_label_groups` (sum_labels/mean/variance/std/min/max/median/labeled_comprehension) moved from O(N·K) per-element `position` scan over K labels to a one-time label→pos HashMap O(N+K): 16.5-88.7x self-speedup (same-binary, growing with K), byte-identical grouping. Still 5-8x slower than SciPy's flat-accumulator C — group materialization is the next lever |
| `fsci-ndimage` label-indexed measurements correctness | guarded | byte-identical (`mism=0` vs verbatim old linear scan in `perf_label_stats`); full `cargo test -p fsci-ndimage --lib` 240 passed / 0 failed |
