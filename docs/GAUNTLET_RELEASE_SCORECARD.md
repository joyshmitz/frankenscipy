...
| `fsci-special` `jnjnp_zeros` performance | **measured NEAR-PARITY (9l5oo)** | cumulative Cephes `j0` + Newton/secant bracket refinement reaches 1.18x slower at `nt=64` (0.589 ms vs SciPy 0.498 ms) and 1.38x slower at `nt=128` (1.239 ms vs 0.897 ms), same-binary SciPy oracle row on local host; flips the worst gap in this ledger (was 163-443x slower at `acoco`) to near parity, 158-381x self-speedup vs the legacy route; earlier cutoff-driven generator was 3.68-3.71x slower, top-k partition rejected as near-noise |
...
| `fsci-ndimage` label-indexed measurements performance | **complexity gap closed; mean constant cut** | `measurement_label_groups` moved shared label reductions from O(N·K) to O(N+K). Follow-ups first removed per-label bucket materialization, then replaced per-element HashMap probes with compact dense integer-label lookup for `mean(labels,index)`: dense is 2.02-2.10x faster than the prior flat HashMap route, 0 mismatches, but still 1.9-2.6x slower than SciPy's compiled C |
| `fsci-ndimage` label-indexed measurements correctness | guarded | byte-identical (`mism=0/0/0` vs old linear scan, previous bucketed route, and previous flat HashMap route in `perf_label_stats`); full `cargo test -p fsci-ndimage --lib` 241 passed / 0 failed; live SciPy-backed `diff_ndimage` conformance passed locally 5/0 |
| `fsci-ndimage` lint/build gate | partial | `cargo check -p fsci-ndimage --all-targets`, touched-file rustfmt, `git diff --check`, UBS, and ndimage-scoped conformance passed; existing `fsci-interpolate`/`diff_geom` warnings, broad full-conformance contract-table/oracle blockers, and unrelated clippy inventories remain outside this patch |
