...
| `fsci-ndimage` lint/build gate | partial | `cargo check -p fsci-ndimage --all-targets`, touched-file rustfmt, `git diff --check`, UBS, and local ndimage-scoped conformance passed; rch live SciPy conformance is blocked by missing worker SciPy, and unrelated clippy inventories remain outside this patch |
