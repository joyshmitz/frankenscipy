...
| `frankenscipy-8l8r1.127` | `distance_transform_edt(return_indices)` exact line starts + no per-cell coordinate allocation | 64x64 return_indices | 216.733 us | 173.434 us | 1.25x slower | keep 1.50x same-worker internal win; route deeper |
| `frankenscipy-8l8r1.127` | `distance_transform_edt(return_indices)` exact line starts + no per-cell coordinate allocation | 128x128 return_indices | 1.207 ms | 775.685 us | 1.56x slower | keep 1.14x same-worker internal win; route deeper |
| `frankenscipy-8l8r1.127` | `distance_transform_edt(return_indices)` exact line starts + no per-cell coordinate allocation | 192x192 return_indices | 2.107 ms | 2.280155 ms | **1.08x faster** | keep 1.81x same-worker internal win; first EDT SciPy win |
| `frankenscipy-8l8r1.127` | `distance_transform_edt(return_indices)` exact line starts + no per-cell coordinate allocation | 256x256 return_indices | 4.855 ms | 4.288605 ms | 1.13x slower | keep 1.21x same-worker internal win; route deeper |
...
| `frankenscipy-8l8r1.129` | 2D Reflect Gaussian cache-planned separable pass | Generic embedded N-D convolution path for the first axis plus line walk where profitable | 1.68x faster on same-worker `vmi1152480` for `gaussian_sigma2/256`; final Rust remains 1.34x slower than SciPy | keep current despite SciPy loss; route deeper |
...
| `fsci-ndimage` `distance_transform_edt` indices performance | **complexity gap closed; mixed SciPy score now `1/3/0`** | `return_indices` path moved from O(foreground*background) brute force to exact separable feature transform, then `frankenscipy-8l8r1.127` removed flat-index line-start scans and per-cell coordinate Vec allocation. Same-worker final rows are 1.50/1.14/1.81/1.21x faster than the prior feature-transform route; final Rust beats SciPy at 192x192 by 1.08x but remains 1.25/1.56/1.13x slower at 64/128/256 |
| `fsci-ndimage` `distance_transform_edt` indices correctness | guarded | `perf_edt` isomorphism 0 mismatches/10876 cells before and after; focused `distance_transform_edt` tests passed via rch 15/0; full ndimage lib tests passed via rch 242/0; live SciPy `diff_ndimage_distance_transform_edt` conformance passed locally 1/0 |
| `fsci-ndimage` label-indexed measurements performance | **complexity gap closed; mixed SciPy score now `1/3/0`** | `measurement_label_groups` moved shared label reductions from O(N*K) to O(N+K). Follow-ups removed per-label bucket materialization, HashMap probes, dense `fract()` classification, and now the common one-based dense table. `frankenscipy-8l8r1.126` one_based is 1.08/1.08/1.36/1.33x faster than reconstructed dense_table on rch `hz2`; same-host final source is 1.23x faster than SciPy on N=65536 K=512 and 1.09/1.19/1.03x slower on the larger rows |
...
| `fsci-ndimage` gaussian_filter route | measured internal keep plus residual SciPy loss | `frankenscipy-8l8r1.129` 2D Reflect cache-planned separable pass cuts `gaussian_sigma2/256` from 3.2989 ms to 1.9680 ms on same-worker `vmi1152480` (1.68x internal win), but remains 1.34x slower than SciPy 1.46523 ms; route next to vectorized dot kernels, transposed scratch, or cache-blocked separable tiles |
| `fsci-ndimage` gaussian_filter gates | guarded with inherited lint/fmt debt | RCH `cargo check -p fsci-ndimage --all-targets` and RCH focused Gaussian tests pass; local live SciPy conformance passes; `git diff --check` and UBS pass. `cargo fmt --check -p fsci-ndimage` is blocked by pre-existing `ndimage_bench.rs`/`diff_fourier.rs` drift, and clippy is blocked before ndimage by pre-existing `fsci-linalg` lints |
