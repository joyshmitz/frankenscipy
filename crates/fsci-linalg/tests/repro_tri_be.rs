use fsci_linalg::{TriangularSolveOptions, TriangularTranspose, solve_triangular};
...
fn test_solve_triangular_transpose_backward_error() {
    ...
    let be = result
        .backward_error
        .expect("backward error should be present");
    assert!(
        be < 1e-10,
        "backward error should be small for transposed solve, got {}",
        be
    );
}
