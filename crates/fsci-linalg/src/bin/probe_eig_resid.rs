    ...
    for &n in &[5usize, 10, 25, 60, 120] {
        ...
        while i < n {
            ...
            if im.abs() > 1e-12 {
                ...
                for row in 0..n {
                    // (A v)_row real/imag
                    let mut ar = 0.0; let mut ai = 0.0;
                    for k in 0..n { ar += a[row][k]*res.eigenvectors[k][i]; ai += a[row][k]*res.eigenvectors[k][i+1]; }
                    let vr = res.eigenvectors[row][i]; let vi = res.eigenvectors[row][i+1];
                    // lambda*v = (re+im i)(vr+vi i) = (re*vr - im*vi) + (re*vi + im*vr) i
                    let lr = re*vr - im*vi; let li = re*vi + im*vr;
                ...
                for row in 0..n {
                    let mut av = 0.0;
                    for k in 0..n { av += a[row][k]*res.eigenvectors[k][i]; }
