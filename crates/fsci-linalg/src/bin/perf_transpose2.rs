//! Median-null-gated A/B for `linalg::transpose` (solve-path helper, now delegating to the parallel
//! transpose_rows): ORIG serial scatter loop vs parallel. Toggled by
//! `LINALG_MAT_ELEMENTWISE_FORCE_SERIAL`. BYTE-IDENTICAL. Args: side [iters].
use fsci_linalg::{LINALG_MAT_ELEMENTWISE_FORCE_SERIAL, transpose};
...
fn main() {
    ...
    println!("# linalg::transpose {side}x{side} a[1][0]={} b[1][0]={} bitmism={bitmism}", a[1][0], b[1][0]);

    let bench = |serial: bool| -> f64 {
        LINALG_MAT_ELEMENTWISE_FORCE_SERIAL.store(serial, Ordering::Relaxed);
        let _ = black_box(transpose(black_box(&m)));
        let t = Instant::now();
        for _ in 0..3 {
            let _ = black_box(transpose(black_box(&m)));
        }
    ...
    println!(
        "{} transpose serial {ob:.2}ms (cv {:.1}%) parallel {fb:.2}ms (cv {:.1}%) | \
