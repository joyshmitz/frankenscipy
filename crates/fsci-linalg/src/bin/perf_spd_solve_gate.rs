...
use fsci_linalg::{solve, MatrixAssumption, SolveOptions, SOLVE_FLAT_MIN_OVERRIDE};
...
fn main() {
    ...
    for &n in &[128usize, 256, 384, 512, 768] {
        ...
        let opts = || { let mut o = SolveOptions::default(); o.assume_a = Some(MatrixAssumption::PositiveDefinite); o };
        ...
        let maxdiff = xp.iter().zip(&xf).map(|(a,b)| (a-b).abs()).fold(0.0f64,f64::max);
        let reps = 6;
        let bench = |ov: usize| { SOLVE_FLAT_MIN_OVERRIDE.store(ov, Ordering::Relaxed);
            let _=black_box(solve(&a,&b,opts()).unwrap());
            let t=Instant::now(); for _ in 0..reps { let _=black_box(solve(black_box(&a),black_box(&b),opts()).unwrap()); }
            t.elapsed().as_secs_f64()/reps as f64*1000.0 };
        let portfolio=bench(1000).min(bench(1000)); let fast=bench(1).min(bench(1));
        println!("spd_solve n={n:4}: portfolio {portfolio:8.2}ms -> fast {fast:8.2}ms = {:.2}x  maxdiff={maxdiff:.1e}", portfolio/fast);
    }
