//! Median-null-gated A/B for `linalg::hadamard_product` (representative of the element-wise matrix
//! family): ORIG serial row-wise map vs row-chunked parallel build. Toggled by
//! `LINALG_MAT_ELEMENTWISE_FORCE_SERIAL`, alternated per iteration. BYTE-IDENTICAL. Args: side [iters].
use fsci_linalg::{LINALG_MAT_ELEMENTWISE_FORCE_SERIAL, hadamard_product};
...
fn main() {
    ...
    let bench = |serial: bool| -> f64 {
        ...
        let _ = black_box(hadamard_product(black_box(&a), black_box(&b)));
        let t = Instant::now();
        for _ in 0..3 {
            let _ = black_box(hadamard_product(black_box(&a), black_box(&b)));
        }
    ...
    println!(
        "{} hadamard_product serial {ob:.2}ms (cv {:.1}%) parallel {fb:.2}ms (cv {:.1}%) | \
