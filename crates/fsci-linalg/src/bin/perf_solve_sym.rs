use fsci_linalg::{solve, SolveOptions, MatrixAssumption};
...
fn main() {
    ...
    for &n in &[256usize, 384, 512] {
        ...
        let mut sym = SolveOptions::default(); sym.assume_a = Some(MatrixAssumption::Symmetric);

        for (name, a) in [("PD", &pd), ("INDEF", &indef)] {
            let xg = solve(a,&b,SolveOptions::default()).unwrap().x;
            let xs = solve(a,&b,sym).unwrap().x;
            let maxdiff = xg.iter().zip(&xs).map(|(a,b)| (a-b).abs()).fold(0.0f64,f64::max);
            let bench = |o: SolveOptions| { let _=black_box(solve(a,&b,o).unwrap()); let reps=6; let t=Instant::now(); for _ in 0..reps { let _=black_box(solve(black_box(a),black_box(&b),o).unwrap()); } t.elapsed().as_secs_f64()/reps as f64*1000.0 };
            let (mut g, mut s) = (f64::MAX, f64::MAX);
            for _ in 0..4 { g = g.min(bench(SolveOptions::default())); s = s.min(bench(sym)); }
            println!("n={n} {name}: General {g:.2}ms  Symmetric {s:.2}ms  ratio(G/S)={:.2}x  maxdiff={maxdiff:.1e}", g/s);
        }
    }
}
