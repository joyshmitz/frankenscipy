...
use fsci_linalg::{DecompOptions, LU_FACTOR_FLAT_MIN_OVERRIDE, lu};
...
fn main() {
    ...
    for &n in &[128usize, 256, 384, 512, 768] {
        ...
        let bench = |ov: usize| {
            ...
            let t = Instant::now();
            for _ in 0..reps {
                let _ = black_box(lu(black_box(&a), DecompOptions::default()).unwrap());
            }
        ...
        println!(
            "lu n={n:4}: nalgebra {nalg:8.2}ms -> blocked {blocked:8.2}ms = {:.2}x",
            nalg / blocked
        );
    }
