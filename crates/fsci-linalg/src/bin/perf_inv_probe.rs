...
use fsci_linalg::{INV_FLAT_MIN_OVERRIDE, InvOptions, inv};
...
fn main() {
    ...
    for &n in &[256usize, 384, 512, 640, 768] {
        ...
        let reps = 4;
        let bench = |ov: usize| {
            INV_FLAT_MIN_OVERRIDE.store(ov, Ordering::Relaxed);
            let _ = black_box(inv(&a, InvOptions::default()).unwrap());
            let t = Instant::now();
            for _ in 0..reps {
                let _ = black_box(inv(black_box(&a), InvOptions::default()).unwrap());
            }
            t.elapsed().as_secs_f64() / reps as f64 * 1000.0
        };
        let portfolio = bench(4096).min(bench(4096));
        let blocked = bench(1).min(bench(1));
        println!(
            "inv n={n:4}: portfolio {portfolio:8.2}ms -> blocked {blocked:8.2}ms = {:.2}x  maxdiff={maxdiff:.1e}",
            portfolio / blocked
        );
    }
