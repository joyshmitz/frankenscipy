...
fn main() {
    ...
    for &n in &[64usize, 256, 512, 1024, 2048] {
        ...
        let mut c: Vec<f64> = (0..n).map(|_| rng() * 0.1).collect();
        c[0] = n as f64;
        let r: Vec<f64> = {
            let mut r: Vec<f64> = (0..n).map(|_| rng() * 0.1).collect();
            r[0] = c[0];
            r
        };
        ...
        let t = Instant::now();
        for _ in 0..reps {
            acc += solve_toeplitz(&c, Some(&r), &b).unwrap()[0];
        }
        let tt = t.elapsed().as_secs_f64() / reps as f64;
        // circulant: positive c[0]-dominant
        let mut cc: Vec<f64> = (0..n).map(|_| rng() * 0.1).collect();
        cc[0] = n as f64;
        let t = Instant::now();
        for _ in 0..reps {
            acc += solve_circulant(&cc, &b).unwrap()[0];
        }
        let tc = t.elapsed().as_secs_f64() / reps as f64;
        println!(
            "n={n:>5}  solve_toeplitz={:>9.1}us  solve_circulant={:>9.1}us  (acc={acc:.2})",
            tt * 1e6,
            tc * 1e6
        );
    }
