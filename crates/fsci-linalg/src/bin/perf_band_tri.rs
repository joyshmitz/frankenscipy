...
use fsci_linalg::{
    SolveOptions, TriangularSolveOptions, solve_banded, solve_triangular, solveh_banded,
};
...
fn main() {
    ...
    for &n in &[64usize, 256, 512, 1024, 2048] {
        ...
        for i in 0..n {
            for j in 0..=i {
                a[i][j] = if i == j { n as f64 } else { rng() * 0.1 };
            }
        }
        let topt = TriangularSolveOptions {
            lower: true,
            ..Default::default()
        };
        ...
        let t = Instant::now();
        for _ in 0..reps {
            acc += solve_triangular(&a, &b, topt).unwrap().x[0];
        }
        ...
        for j in 0..n {
            for i in j.saturating_sub(u)..=(j + l).min(n - 1) {
                let r = u + i - j;
                ab[r][j] = if i == j {
                    (n as f64) * 2.0
                } else {
        ...
        let t = Instant::now();
        for _ in 0..reps {
            acc += solve_banded((l, u), &ab, &b, SolveOptions::default())
                .unwrap()
                .x[0];
        }
        ...
        let t = Instant::now();
        for _ in 0..reps {
            acc += solveh_banded(&abh, &b, true).unwrap().x[0];
        }
        let tsh = t.elapsed().as_secs_f64() / reps as f64;

        println!(
            "n={n:>5}  solve_triangular={:>8.1}us  solve_banded={:>8.1}us  solveh_banded={:>8.1}us  (acc={acc:.2})",
            ttri * 1e6,
            tband * 1e6,
            tsh * 1e6
        );
