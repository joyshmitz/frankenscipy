...
use fsci_linalg::{solve, SolveOptions, SOLVE_FLAT_MIN_OVERRIDE};
...
fn main() {
    ...
    for &n in &[32usize, 64, 96, 128, 192, 256] {
        ...
        let bench = |ov: usize| { SOLVE_FLAT_MIN_OVERRIDE.store(ov, Ordering::Relaxed);
            ...
            let t=Instant::now(); for _ in 0..reps { let _=black_box(solve(black_box(&a),black_box(&b),SolveOptions::default()).unwrap()); }
            t.elapsed().as_secs_f64()/reps as f64*1000.0 };
        let portfolio=bench(0).min(bench(0)); let fast=bench(128).min(bench(128));
        println!("solve n={n:4}: portfolio {portfolio:8.2}ms -> fast {fast:8.2}ms = {:.2}x  maxdiff={maxdiff:.1e}", portfolio/fast);
    }
    SOLVE_FLAT_MIN_OVERRIDE.store(0, Ordering::Relaxed);
}
