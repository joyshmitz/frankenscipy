...
use fsci_linalg::{lu_factor, DecompOptions, LU_FACTOR_FLAT_MIN_OVERRIDE};
...
fn main() {
    ...
    for &n in &[128usize, 256, 384, 512, 768] {
        ...
        let bench = |ov: usize| { LU_FACTOR_FLAT_MIN_OVERRIDE.store(ov, Ordering::Relaxed);
            ...
            let t=Instant::now(); for _ in 0..reps { let _=black_box(lu_factor(black_box(&a),DecompOptions::default()).unwrap()); }
            t.elapsed().as_secs_f64()/reps as f64*1000.0 };
        let nalg=bench(4096).min(bench(4096)); let blocked=bench(1).min(bench(1));
        println!("lu_factor n={n:4}: nalgebra {nalg:8.2}ms -> blocked {blocked:8.2}ms = {:.2}x", nalg/blocked);
    }
