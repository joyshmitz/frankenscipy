//! Median-null-gated A/B for `SmoothBivariateSpline::eval_many` (scattered points): the ORIG per-query
//! path (rebuilds the ny x-direction BSplines on every query, serially) vs the hoisted + parallel path
//! (builds the x-splines once, reuses across the batch, fans the independent per-query evals across
//! cores via `par_query_map`). Both arms live in ONE binary, toggled by
//! `SMOOTHBISPLINE_EVAL_MANY_FORCE_SCALAR` and ALTERNATED per iteration inside one measured routine, so
//! a single `rch exec` invocation measures both on the same worker. `bitmism` proves byte-identity.
use fsci_interpolate::{
    SMOOTHBISPLINE_EVAL_MANY_FORCE_SCALAR, SmoothBivariateSpline, SmoothBivariateSplineOptions,
};
...
fn main() {
    ...
    let nq: usize = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(20000); // scattered queries
    ...
    // Scattered samples on a jittered grid (matches perf_smoothbispline_evalgrid's fixture).
    ...
    // Parity: hoisted+parallel must be byte-identical to the serial per-query path.
    ...
    let bench = |force_scalar: bool| -> f64 {
        SMOOTHBISPLINE_EVAL_MANY_FORCE_SCALAR.store(force_scalar, Ordering::Relaxed);
    ...
    println!("# SmoothBivariateSpline eval_many {nsamp}x{nsamp} samples, {nq} scattered queries");
    println!(
        "{} scalar {ob:.2}ms (cv {:.1}%) hoisted+par {hb:.2}ms (cv {:.1}%) | CAND(scalar/new) median \
