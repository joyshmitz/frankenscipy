...
fn main() {
    ...
    for &n in &[150usize, 300, 600] {
        ...
        let t=Instant::now(); let reps=3; let mut acc=0.0;
        for _ in 0..reps { let rbf = RbfInterpolator::new(black_box(&pts),black_box(&vals),RbfKernel::Multiquadric,1.0).unwrap(); acc+=rbf.eval(&[0.5,0.5,0.5]); }
        println!("n={n}: rbf_build {:.1} ms (acc={acc:.3})", t.elapsed().as_secs_f64()/reps as f64*1000.0);
    }
}
