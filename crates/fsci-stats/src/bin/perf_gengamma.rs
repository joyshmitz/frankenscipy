use fsci_stats::{GENGAMMA_LN_REUSE_DISABLE, GenGamma};
...
fn main() {
    ...
    for &(a, c) in &[(2.5f64, 1.7f64), (1.3, 0.8), (3.1, 2.4)] {
        ...
        let bench = |disable: bool| {
            ...
            let t = Instant::now();
            for _ in 0..reps {
                let _ = black_box(g.pdf_many(black_box(&xs)));
            }
            t.elapsed().as_secs_f64() / reps as f64 * 1000.0
        };
        let (mut re, mut og) = (f64::MAX, f64::MAX);
        for _ in 0..4 {
            og = og.min(bench(true));
            re = re.min(bench(false));
        }
        println!(
            "a={a} c={c}: orig(powf) {og:.2}ms  reuse(ln) {re:.2}ms  speedup {:.2}x  maxrel={maxrel:.1e}",
            og / re
        );
    }
}
