//! Median-null-gated A/B for `stats::zscore_weighted`: ORIG separate finite-check + Σw + weighted-
//! mean passes vs one fused pass over (data,weights). Toggled by `ZSCORE_W_FUSE_DISABLE`, alternated
//! per iteration. BYTE-IDENTICAL. Args: n [iters].
use fsci_stats::{ZSCORE_W_FUSE_DISABLE, zscore_weighted};
...
fn main() {
    ...
    let bench = |disable: bool| -> f64 {
        ...
        let _ = black_box(zscore_weighted(black_box(&data), black_box(&w)));
        let t = Instant::now();
        for _ in 0..3 {
            let _ = black_box(zscore_weighted(black_box(&data), black_box(&w)));
        }
    ...
    println!(
        "{} zscore_weighted orig {ob:.2}ms (cv {:.1}%) fused {fb:.2}ms (cv {:.1}%) | \
