//! Median-null-gated A/B for `stats::pmean`: the ORIG fused serial `map(powf).sum()` vs the
...
//! toggled by `PMEAN_FORCE_SERIAL` and ALTERNATED per iteration. BYTE-IDENTICAL (bitmism=0 expected):
//! the map values are independent and the left-fold order is preserved. Peer: scipy.stats.pmean.
use fsci_stats::{PMEAN_FORCE_SERIAL, pmean};
...
fn main() {
    ...
    PMEAN_FORCE_SERIAL.store(true, Ordering::Relaxed);
    let a = pmean(&data, p);
    PMEAN_FORCE_SERIAL.store(false, Ordering::Relaxed);
    let b = pmean(&data, p);
    let bitmism = usize::from(a.to_bits() != b.to_bits());

    let bench = |force_serial: bool| -> f64 {
        PMEAN_FORCE_SERIAL.store(force_serial, Ordering::Relaxed);
        let run = || pmean(black_box(&data), black_box(p));
        let _ = black_box(run());
        let t = Instant::now();
        for _ in 0..3 {
            let _ = black_box(run());
    ...
    println!("# stats::pmean {n} elements, p={p} (result serial={a} parallel={b})");
    println!(
