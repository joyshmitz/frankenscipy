...
use fsci_stats::{energy_distance, ks_2samp, wasserstein_distance, RANKDATA_RADIX_DISABLE};
    ...
    for &n in &[50_000usize, 200_000, 1_000_000] {
        ...
        for (name, f) in cases {
            let bench = |disable: bool| {
                ...
                let t = Instant::now();
                for _ in 0..reps { let _ = black_box(f(black_box(&u), black_box(&v))); }
                t.elapsed().as_secs_f64() / reps as f64 * 1000.0
            };
            let base = bench(true).min(bench(true));
            let rdx = bench(false).min(bench(false));
            println!("n={n:8} {name:12}: sort {base:8.2}ms -> radix {rdx:8.2}ms = {:.2}x", base / rdx);
        }
    }
