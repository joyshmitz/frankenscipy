//! Median-null-gated A/B for `stats::zmap_ddof`: ORIG serial `(score-mean)/std` map over scores vs
//! parallel output map (mean_std_ddof(compare) stays serial). Toggled by `ZSCORE_FORCE_SERIAL` (shared
//! zscore-family knob). BYTE-IDENTICAL. Args: n [iters].
use fsci_stats::{ZSCORE_FORCE_SERIAL, zmap_ddof};
...
fn main() {
    ...
    let scores: Vec<f64> = (0..n).map(|_| r()).collect();
    let compare: Vec<f64> = (0..n).map(|_| r()).collect();

    ZSCORE_FORCE_SERIAL.store(true, Ordering::Relaxed);
    ...
    let bench = |serial: bool| -> f64 {
        ...
        let _ = black_box(zmap_ddof(black_box(&scores), black_box(&compare), 0));
        let t = Instant::now();
        for _ in 0..3 {
            let _ = black_box(zmap_ddof(black_box(&scores), black_box(&compare), 0));
        }
    ...
    println!(
        "{} zmap_ddof serial {ob:.2}ms (cv {:.1}%) parallel {fb:.2}ms (cv {:.1}%) | \
