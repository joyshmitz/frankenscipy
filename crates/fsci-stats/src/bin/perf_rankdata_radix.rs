...
use fsci_stats::{rankdata, RANKDATA_RADIX_DISABLE};
...
fn main() {
    ...
    for &n in &[50_000usize, 200_000, 1_000_000, 4_000_000] {
        ...
        let data: Vec<f64> = (0..n).map(|i| if i % 8 == 0 { (i % 1000) as f64 } else { r() * 1e3 - 500.0 }).collect();
        for method in ["average", "ordinal"] {
            ...
            println!("n={n:8} {method:8}: sort {base:8.2}ms -> radix {rdx:8.2}ms = {:.2}x", base / rdx);
        }
    }
}
