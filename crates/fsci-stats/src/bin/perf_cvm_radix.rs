use fsci_stats::{cramervonmises_2samp, RANKDATA_RADIX_DISABLE};
...
fn main() {
    ...
    for &n in &[50_000usize, 200_000, 1_000_000] {
        ...
        let idok = a.statistic.to_bits()==b.statistic.to_bits() && a.pvalue.to_bits()==b.pvalue.to_bits();
        let reps = if n<=200_000 {8} else {3};
        let bench = |d: bool| { RANKDATA_RADIX_DISABLE.store(d, Ordering::Relaxed);
            let _=black_box(cramervonmises_2samp(&x,&y));
            let t=Instant::now(); for _ in 0..reps { let _=black_box(cramervonmises_2samp(black_box(&x),black_box(&y))); }
            t.elapsed().as_secs_f64()/reps as f64*1000.0 };
        let base=bench(true).min(bench(true)); let rdx=bench(false).min(bench(false));
        println!("cvm2 n={n:8}: sort {base:8.2}ms -> radix {rdx:8.2}ms = {:.2}x  bitid={idok}", base/rdx);
    }
    RANKDATA_RADIX_DISABLE.store(false, Ordering::Relaxed);
}
