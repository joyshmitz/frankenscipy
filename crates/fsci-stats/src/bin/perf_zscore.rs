//! Median-null-gated A/B for `stats::zscore`: ORIG serial `(x-mean)/std` map vs parallel output map
//! (mean_std_ddof stays serial). Toggled by `ZSCORE_FORCE_SERIAL`. BYTE-IDENTICAL. Args: n [iters].
use fsci_stats::{ZSCORE_FORCE_SERIAL, zscore};
...
fn main() {
    ...
    println!("# stats::zscore n={n} a[1]={} b[1]={} bitmism={bitmism}", a[1], b[1]);

    let bench = |serial: bool| -> f64 {
        ZSCORE_FORCE_SERIAL.store(serial, Ordering::Relaxed);
        let _ = black_box(zscore(black_box(&data)));
        let t = Instant::now();
        for _ in 0..3 {
            let _ = black_box(zscore(black_box(&data)));
        }
    ...
    println!(
        "{} zscore serial {ob:.2}ms (cv {:.1}%) parallel {fb:.2}ms (cv {:.1}%) | \
