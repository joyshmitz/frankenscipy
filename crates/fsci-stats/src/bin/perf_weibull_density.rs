use fsci_stats::{ContinuousDistribution, WEIBULL_DENSITY_REUSE_DISABLE, Weibull};
    ...
    for &(c, scale) in &[(1.5f64, 2.0f64), (0.8, 1.0), (3.0, 1.5)] {
        ...
        for which in ["pdf", "logpdf"] {
            let eval = |disable: bool| -> Vec<f64> {
                ...
                if which == "pdf" {
                    xs.iter().map(|&x| w.pdf(x)).collect()
                } else {
                    xs.iter().map(|&x| w.logpdf(x)).collect()
                }
            };
            let re = eval(false);
            let og = eval(true);
            ...
            let bench = |disable: bool| {
                ...
                let f = || -> f64 {
                    if which == "pdf" {
                        xs.iter().map(|&x| w.pdf(x)).sum()
                    } else {
                        xs.iter().map(|&x| w.logpdf(x)).sum()
                    }
                };
                let _ = black_box(f());
                let reps = 6;
            ...
            println!(
                "c={c} scale={scale} {which}: orig {ogt:.2}ms  reuse {ret:.2}ms  speedup {:.2}x  maxrel={maxrel:.1e}",
                ogt / ret
            );
        }
    }
