//! Median-null-gated A/B for `stats::idealfourths`: the ORIG full O(n log n) sort vs quickselecting
//! the four needed order statistics via `select_ranks` (O(n)). Toggled by `IDEALFOURTHS_FORCE_SORT`,
//! alternated per iteration. BYTE-IDENTICAL (same order statistics + interpolation). Args: n [iters].
use fsci_stats::{IDEALFOURTHS_FORCE_SORT, idealfourths};
...
fn main() {
    ...
    IDEALFOURTHS_FORCE_SORT.store(true, Ordering::Relaxed);
    ...
    let bench = |force_sort: bool| -> f64 {
        IDEALFOURTHS_FORCE_SORT.store(force_sort, Ordering::Relaxed);
        let _ = black_box(run());
        let t = Instant::now();
        for _ in 0..3 {
            let _ = black_box(run());
    ...
    println!(
        "{} idealfourths full-sort {ob:.2}ms (cv {:.1}%) quickselect {fb:.2}ms (cv {:.1}%) | \
