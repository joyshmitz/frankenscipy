//! Median-null-gated A/B for the shared label value/position GATHER
//! (`measurement_label_value_positions`), measured through `ndimage::extrema` (a gather-dominated
//! caller: its per-group min/max/argmin/argmax scans are light, so the per-element SipHash bucketing
//! dominates). ORIG serial per-element bucketing vs the across-elements privatized-buckets + merge.
//! Both arms live in ONE binary, toggled by `NDIMAGE_LABEL_GATHER_FORCE_SERIAL` and ALTERNATED per
//! iteration inside one measured routine, so a single `rch exec` invocation measures both on the same
//! worker. This lever also compounds the already-parallel `labeled_comprehension` map.
use fsci_ndimage::{NDIMAGE_LABEL_GATHER_FORCE_SERIAL, NdArray, extrema};
...
fn main() {
    ...
    // Parity: all four output vectors must be bit-identical across the two gather arms.
    ...
    let bench = |force_serial: bool| -> f64 {
        ...
        let run = || extrema(black_box(&input), Some(black_box(&labels)), Some(&index)).unwrap();
        let _ = black_box(run());
        let t = Instant::now();
        for _ in 0..3 {
            let _ = black_box(run());
    ...
    println!("# ndimage label gather via extrema {npix} pixels, {nlabels} labels");
    println!(
