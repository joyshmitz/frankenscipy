//! Median-null-gated A/B for `ndimage::abs_array`: ORIG serial element-wise abs map vs parallel fill.
//! Cheap map — tests bandwidth aggregation. Toggled by `NDIMAGE_ABS_ARRAY_FORCE_SERIAL`. BYTE-IDENTICAL.
//! Args: n [iters].
use fsci_ndimage::{NDIMAGE_ABS_ARRAY_FORCE_SERIAL, NdArray, abs_array};
...
fn main() {
    ...
    NDIMAGE_ABS_ARRAY_FORCE_SERIAL.store(true, Ordering::Relaxed);
    let a = abs_array(&arr);
    ...
    let bench = |serial: bool| -> f64 {
        ...
        let _ = black_box(abs_array(black_box(&arr)));
        let t = Instant::now();
        for _ in 0..5 {
            let _ = black_box(abs_array(black_box(&arr)));
        }
    ...
    println!(
        "{} abs_array serial {ob:.2}ms (cv {:.1}%) parallel {fb:.2}ms (cv {:.1}%) | \
