//! Median-null-gated A/B for `signal::magnitude_response`: the ORIG serial per-frequency loop vs the
//! parallel-across-frequencies path (via the shared `freqz_par_collect` helper). Both arms live in
//! ONE binary, toggled by `MAGNITUDE_RESPONSE_FORCE_SERIAL` and ALTERNATED per iteration.
//!
//! Each frequency's magnitude is a pure function of its index: two Horner `eval_poly_on_unit_circle`
//! sweeps (O(len(b)+len(a)) complex MACs + a cos/sin each) plus a `sqrt` — compute-bound at high
//! filter order and independent per frequency. Last straggler of the response family
//! (`group_delay_from_ba`/`phase_response` landed); `magnitude_response_db` wraps this fn.
use fsci_signal::{MAGNITUDE_RESPONSE_FORCE_SERIAL, magnitude_response};
...
fn main() {
    ...
    MAGNITUDE_RESPONSE_FORCE_SERIAL.store(true, Ordering::Relaxed);
    let (fa, ma) = magnitude_response(&b, &a, n_freqs);
    ...
    let bench = |force_serial: bool| -> f64 {
        MAGNITUDE_RESPONSE_FORCE_SERIAL.store(force_serial, Ordering::Relaxed);
        let run = || magnitude_response(black_box(&b), black_box(&a), n_freqs);
        let _ = black_box(run());
        let t = Instant::now();
        for _ in 0..5 {
            let _ = black_box(run());
    ...
    println!("# signal::magnitude_response order={order} n_freqs={n_freqs}");
    println!(
