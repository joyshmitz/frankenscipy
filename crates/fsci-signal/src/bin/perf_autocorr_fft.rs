use fsci_signal::{AUTOCORR_FFT_DISABLE, autocorrelation};
...
fn main() {
    ...
    for &(n, lag) in &[
        ...
        let bench = |d: bool| {
            AUTOCORR_FFT_DISABLE.store(d, Ordering::Relaxed);
        ...
        let dir = bench(true).min(bench(true));
        let fftp = bench(false).min(bench(false));
        println!(
            "n={n:6} lag={lag:6}: direct {dir:9.3}ms -> fft {fftp:9.3}ms = {:.2}x",
            dir / fftp
        );
    }
