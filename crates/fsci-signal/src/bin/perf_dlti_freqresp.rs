//! Median-null-gated A/B for `signal::Dlti::freqresp`: the ORIG serial per-frequency loop vs the
...
//!
//! Each frequency's response H(e^{jωdt})=num/den is a pure function of its index: `eval_at_freq` does
...
fn main() {
    ...
    // Realistic high-order discrete-time transfer function (deterministic pseudo-random).
    ...
    println!("# signal::Dlti::freqresp order={order} n_freqs={n_freqs}");
    println!(
