...
fn xin(n: usize) -> Vec<(f64,f64)> {
    (0..n).map(|k| ((0.3*k as f64+1.0).sin(), (0.17*k as f64).cos())).collect()
}
fn pr(tag: &str, v: &[(f64,f64)]) {
    print!("{tag}");
    for c in v { print!(" {:.12} {:.12}", c.0, c.1); }
    println!();
}
fn main() {
    ...
    let wm=0.99f64; let wa=-2.0*std::f64::consts::PI/12.0;
    let w=(wm*wa.cos(), wm*wa.sin());
    let am=1.1f64; let aa=0.1f64; let a=(am*aa.cos(), am*aa.sin());
