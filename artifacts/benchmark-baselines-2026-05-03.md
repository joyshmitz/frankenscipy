...
`baseline_*.json` now ships with a `<file>.raptorq.json` sidecar generated by the new
`raptorq_sidecar` binary in this same crate (added under bead frankenscipy-010ms). The
sidecar contains:

- `source_hash`: BLAKE3 of the file bytes
- `symbol_size`, `source_symbols`, `repair_symbols`: RaptorQ encoding shape
- `repair_symbol_hashes` + `repair_symbol_payloads_hex`: enough repair information that the
  source file can be recovered if a single source symbol is lost

Regeneration workflow:

```bash
cargo run --release -p fsci-conformance --bin raptorq_sidecar -- \
    docs/baseline_linalg.json docs/baseline_fft.json docs/baseline_opt.json \
    docs/baseline_sparse.json docs/baseline_integrate.json
```

CI verification (run after a fresh checkout to detect bit-rot):

```bash
cargo run --release -p fsci-conformance --bin raptorq_sidecar -- --verify \
    docs/baseline_*.json
```

Verification fails (exit code 1) if the source file's `source_hash` no longer matches the
sidecar; that is the durability tripwire.
